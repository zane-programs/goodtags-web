...
- [cloudflare-deployment.md](cloudflare-deployment.md) — how the app runs on Cloudflare: Worker + static assets, the media bridge's edge-cache and range invariants, cost model, `infra/` (OpenTofu) vs `wrangler.jsonc` ownership, first deploy, CI deploys and operations. **Read before changing `worker/`, `wrangler.jsonc`, `infra/` or `public/_headers`.** Committed infra must stay deployer-agnostic (no account IDs, zones or hostnames).
